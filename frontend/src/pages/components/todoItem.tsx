...
export function TodoItem({todo, getTask, folder}:TodoProps){
    ...
    const changeTitleEditMode = (title:string) => {
        setTitleEditMode(!titleEditMode);
        setTitle(title);
    }

    const changeDate = async (date:string, id:string) => {
        if (!date) return setDateEditMode(!dateEditMode);
        ...
        await axios({
            ...
            withCredentials: true,
            url: "http://localhost:4000/changeDateTask"
        });
    ...
    const changeDateEditMode = () => {
        setDateEditMode(!dateEditMode);
    }

    const date = () => {
        let date = new Date(Number(todo.date));
